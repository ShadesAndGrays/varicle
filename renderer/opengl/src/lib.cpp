...
#include "renderer/opengl/lib.hpp"
#include "renderer/opengl/util.hpp"
...
void bind_texture(GLContext& ctx, uint32_t texture) {
...
void load_default_shader(GLContext& ctx) {
    auto result = load_shader(default_vert_shader, default_frag_shader);
...
GLContext init(Window& window) {
    ...
    glfwSetFramebufferSizeCallback(
        ctx.window.get_window(), framebuffer_resize_callback
    );
